	...
	const struct bt_cap_commander_broadcast_reception_stop_param *param)
{
	...
	/* TODO the following should be a function, since it is shared with the start check */
	for (size_t i = 0; i < param->count; i++) {
		...
		CHECKIF(stop_param->subgroups == NULL) {
			LOG_DBG("param->param[%zu]->subgroup is NULL", i);
			return false;
		}

		total_bis_sync = 0U;
		for (size_t j = 0U; j < stop_param->num_subgroups; j++) {
			const struct bt_bap_bass_subgroup *param_subgroups =
				&stop_param->subgroups[j];
			...
			if (param_subgroups->bis_sync != BT_BAP_BIS_SYNC_NO_PREF) {
				total_bis_sync |= param_subgroups->bis_sync;
			}

			CHECKIF(param_subgroups->metadata_len != 0) {
				LOG_DBG("param->param[%zu].subgroup[%zu].metadata_len must be 0", i,
					j);
	...
	const struct bt_cap_commander_broadcast_reception_stop_param *param)
{
	...
	for (size_t i = 0U; i < param->count; i++) {
		...
		memcpy(stored_param->broadcast_reception_stop.subgroups, member_param->subgroups,
		       sizeof(struct bt_bap_bass_subgroup) * member_param->num_subgroups);
	}
