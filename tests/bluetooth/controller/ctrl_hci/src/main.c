 ...
 *    |                            |                   |
 */
ZTEST(hci_interface, test_hci_feature_exchange)
{
	uint64_t err;
...
ZTEST(hci_interface, test_hci_feature_exchange_wrong_handle)
{
	uint16_t conn_handle;
	uint64_t err;
	int ctx_counter;
...
ZTEST(hci_interface, test_hci_version_ind)
{
	uint64_t err;
	uint16_t conn_handle;
	struct node_tx *tx;
...
ZTEST(hci_interface, test_hci_version_ind_wrong_handle)
{
	uint16_t conn_handle;
	uint64_t err;
	int ctx_counter;
...
ZTEST(hci_interface, test_hci_apto)
{
	uint16_t conn_handle;
	uint64_t err;
...
ZTEST(hci_interface, test_hci_phy)
{
	uint16_t conn_handle;
	uint64_t err;
...
ZTEST(hci_interface, test_hci_dle)
{
	uint16_t conn_handle;
	uint64_t err;
...
ZTEST(hci_interface, test_hci_terminate)
{
	uint16_t conn_handle;
	uint64_t err;
...
ZTEST(hci_interface, test_hci_conn_update)
{
	uint16_t conn_handle;
	uint8_t err;
...
ZTEST(hci_interface, test_hci_chmap)
{
...
ZTEST(hci_interface, test_hci_rssi)
{
	uint16_t conn_handle;
	uint64_t err;
...
ZTEST(hci_interface, test_hci_enc)
{
#ifndef ULL_CENTRAL_MOCKED
	uint16_t conn_handle;
	uint64_t err;
...
ZTEST_SUITE(hci_interface, NULL, NULL, hci_setup, NULL, NULL);
